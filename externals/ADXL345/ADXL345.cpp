...
void ADXL345::read(double *x, double *y, double *z, double& xAcc, double& yAcc, double& zAcc)
{
        byte buffer[6];

        readRegister(ADXL345::DATAX0, 6, buffer);
        xAcc = (buffer[0] + (buffer[1] << 8));
        yAcc = (buffer[2] + (buffer[3] << 8));
        zAcc = (buffer[4] + (buffer[5] << 8));
        *x = (xAcc - zG[0])/256.0;
        *y = (yAcc - zG[1])/256.0;
        *z = (zAcc - zG[2])/256.0;
}
